...
export function createSandboxRouter(
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod', tags: ['sandboxes'], summary: '从 PodSpec 创建 Pod', responses: { 201: { description: 'Pod created', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/pod', tags: ['sandboxes'], summary: '列出所有 Pod', responses: { 200: { description: '{ items, nextCursor }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/pod/{id}', tags: ['sandboxes'], summary: '获取 Pod 详情', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'PodEntity', content: { 'application/json': { schema: z.any() } } }, 404: { description: 'Not found' } } }), async (c) => {
    if (!podService) notConfigured();
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod/{id}/stop', tags: ['sandboxes'], summary: '停止 Pod', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: '{ podId, phase }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
  ...
  app.openapi(createRoute({ method: 'delete', path: '/pod/{id}', tags: ['sandboxes'], summary: '终止 Pod', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Deleted', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod/{id}/sync', tags: ['sandboxes'], summary: '同步 Pod 运行状态', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'PodEntity', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'update', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod/{id}/start', tags: ['sandboxes'], summary: '启动 Pod', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: '{ podId, phase }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'update', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod/{id}/restart', tags: ['sandboxes'], summary: '重启 Pod', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: '{ podId, phase }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'update', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'get', path: '/pod/{id}/health', tags: ['sandboxes'], summary: 'Pod 容器健康状态', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'PodHealth[]', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'read', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'get', path: '/pod/{id}/logs', tags: ['sandboxes'], summary: 'Pod 容器日志', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'ContainerLogResult', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'read', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'post', path: '/pod/{id}/exec', tags: ['sandboxes'], summary: 'Pod 容器执行命令', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Exec result', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'update', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'patch', path: '/pod/{id}', tags: ['sandboxes'], summary: '部分更新 PodSpec', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'PodEntity', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    if (!podService) notConfigured();
    const podId = createPodId(c.req.param('id'));
    const pod = await podService.getById(podId);
    await requirePerm(c, permissionChecker, 'update', 'pod', pod?.creatorId);
  ...
  app.openapi(createRoute({ method: 'get', path: '/', tags: ['sandboxes'], summary: '列出所有沙箱', responses: { 200: { description: '{ items, nextCursor }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}', tags: ['sandboxes'], summary: '获取沙箱详情', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sandbox', content: { 'application/json': { schema: z.any() } } }, 404: { description: 'Not found' } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'post', path: '/{id}/stop', tags: ['sandboxes'], summary: '停止沙箱', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sandbox', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
  ...
  app.openapi(createRoute({ method: 'post', path: '/{id}/start', tags: ['sandboxes'], summary: '启动沙箱', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sandbox', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
    await requirePerm(c, permissionChecker, 'update', 'sandbox', sandbox?.config?.creatorId);
  ...
  app.openapi(createRoute({ method: 'delete', path: '/{id}', tags: ['sandboxes'], summary: '删除沙箱', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Deleted', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
  ...
  app.openapi(createRoute({ method: 'post', path: '/{id}/sync', tags: ['sandboxes'], summary: '同步沙箱状态', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sync result', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
    await requirePerm(c, permissionChecker, 'update', 'sandbox', sandbox?.config?.creatorId);
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/health', tags: ['sandboxes'], summary: '容器健康状态', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'ContainerHealth[]', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
    await requirePerm(c, permissionChecker, 'read', 'sandbox', sandbox?.config?.creatorId);
  ...
  app.openapi(createRoute({ method: 'post', path: '/{id}/restart', tags: ['sandboxes'], summary: '重启沙箱', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sandbox', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
    await requirePerm(c, permissionChecker, 'update', 'sandbox', sandbox?.config?.creatorId);
  ...
  app.openapi(createRoute({ method: 'patch', path: '/{id}', tags: ['sandboxes'], summary: '更新沙箱规格', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Sandbox', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createSandboxId(c.req.param('id'));
    const sandbox = await svc.getById(id);
    await requirePerm(c, permissionChecker, 'update', 'sandbox', sandbox?.config?.creatorId);
