...
export function createSandboxRouter(
  ...
  // ─── Container group (Pod) API — K8s-aligned ───

  // POST /pod — create a pod from K8s-aligned PodSpec
  router.post('/pod', async (c) => {
    ...
    try {
      ...
      const result = await groupProvider.createPod(spec);
      return c.json(ok({ providerId: result.providerId, podName: spec.metadata.name }), 201);
    } catch (e: any) {
      return c.json(fail('POD_CREATE_FAILED', e.message), 500);
    }
  });

  // GET /pod — list all container groups
  ...
  // GET /pod/:providerId — get a single pod
...
export const sandboxRouteMeta: RouteMeta[] = [
  { method: 'POST', path: '/pod', description: '从 PodSpec (K8s-aligned) 创建容器组，返回 providerId', requestBody: { metadata: { name: 'my-pod' }, spec: { containers: [{ name: 'nginx', image: 'nginx:latest' }] } }, responseDescription: '{ providerId, podName }' },
  { method: 'GET', path: '/pod', description: '列出所有容器组（Pod）', responseDescription: 'DescribeContainerGroupsResult' },
  { method: 'GET', path: '/pod/:providerId', description: '获取容器组详情', responseDescription: 'ContainerGroupRuntime' },
  { method: 'POST', path: '/pod/:providerId/stop', description: '停止容器组', responseDescription: '{ ok: true }' },
  { method: 'DELETE', path: '/pod/:providerId', description: '删除容器组', responseDescription: '{ ok: true }' },
  { method: 'GET', path: '/', description: '列出所有沙箱（支持 ?status=&limit=&cursor= 过滤）', responseDescription: '{ items: Sandbox[], nextCursor }' },
  { method: 'GET', path: '/:id', description: '获取沙箱详情', responseDescription: 'Sandbox' },
