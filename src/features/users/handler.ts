...
function userToResponse(user: { id: string; email: string; name: string; role: UserRole; uid?: number; gid?: number; gecos?: string; directory?: string; shell?: string; supplementaryGids?: number[]; createdAt: number; updatedAt: number }): UserResponse {
  return UserResponseSchema.parse({
    ...
    uid: (user as any).uid ?? 1000, gid: (user as any).gid ?? 1000,
    gecos: (user as any).gecos ?? user.name, directory: (user as any).directory ?? `/home/${user.email}`,
    shell: (user as any).shell ?? '/bin/bash', supplementaryGids: (user as any).supplementaryGids ?? [],
...
export function createUserRouter(userService: IUserService, permissionChecker?: PermissionCheckFn): OpenAPIHono {
  ...
  app.openapi(createRoute({ method: 'post', path: '/login', tags: ['users'], summary: '用户登录', request: { body: { content: { 'application/json': { schema: LoginUserSchema } } } }, responses: { 200: { description: 'LoginResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/login-info', tags: ['users'], summary: '查询邮箱的登录方式', responses: { 200: { description: 'LoginInfo', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}', tags: ['users'], summary: '获取用户', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'UserResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'post', path: '/{id}/refresh', tags: ['users'], summary: '刷新用户缓存', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'UserResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
    const now = Date.now();
  ...
  app.openapi(createRoute({ method: 'get', path: '/', tags: ['users'], summary: '列出所有用户', responses: { 200: { description: 'UserResponse[]', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/search', tags: ['users'], summary: '搜索用户', responses: { 200: { description: 'User | null', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/login-policy', tags: ['users'], summary: '获取用户登录策略', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'LoginPolicy', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
    const user = await userService.getById(id);
    if (!user) throw new AppError(404, 'USER_NOT_FOUND', 'User not found');
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/public-key', tags: ['users'], summary: '获取用户 Ed25519 公钥', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'string | null', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
    const user = await userService.getById(id);
    if (!user) throw new AppError(404, 'USER_NOT_FOUND', 'User not found');
  ...
  app.openapi(createRoute({ method: 'put', path: '/{id}/avatar', tags: ['users'], summary: '上传用户头像', request: { params: z.object({ id: z.string() }) }, responses: { 201: { description: '{ size, contentType }', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const user = c.var.currentUser;
  ...
  app.openapi(createRoute({ method: 'delete', path: '/{id}/avatar', tags: ['users'], summary: '删除用户头像', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Deleted', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const user = c.var.currentUser;
    if (!user) throw new AppError(401, 'UNAUTHORIZED', 'Authentication required');
    const rawId = c.req.param('id');
    let targetId: ReturnType<typeof createUserId>;
  ...
  app.openapi(createRoute({ method: 'get', path: '/sessions', tags: ['users'], summary: '列出活跃 session', responses: { 200: { description: 'Session[]', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const user = c.var.currentUser;
    if (!user) throw new AppError(401, 'UNAUTHORIZED', 'Authentication required');
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/supplementary-groups', tags: ['users'], summary: '列出用户辅助组 GID', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'number[]', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
    const gids = await userService.listSupplementaryGroups(id);
    return c.json(ok(gids));
  });

  app.openapi(createRoute({ method: 'put', path: '/{id}/supplementary-groups/{gid}', tags: ['users'], summary: '添加辅助组', request: { params: z.object({ id: z.string(), gid: z.string() }) }, responses: { 200: { description: 'UserResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
  ...
  app.openapi(createRoute({ method: 'delete', path: '/{id}/supplementary-groups/{gid}', tags: ['users'], summary: '移除辅助组', request: { params: z.object({ id: z.string(), gid: z.string() }) }, responses: { 200: { description: 'UserResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
    const id = createUserId(c.req.param('id'));
    const gid = parseInt(c.req.param('gid'));
    if (isNaN(gid) || gid < 0) throw new AppError(400, 'VALIDATION_ERROR', 'Invalid GID');
    { const r = await requirePerm(c, permissionChecker, 'update', 'user', id); if (r) return r; }
  ...
  app.openapi(createRoute({ method: 'post', path: '/no-password-login', tags: ['users'], summary: '无密码登录', request: { body: { content: { 'application/json': { schema: NoPasswordLoginSchema } } } }, responses: { 200: { description: 'LoginResponse', content: { 'application/json': { schema: z.any() } } } } }), async (c) => {
