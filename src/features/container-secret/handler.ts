...
export function createContainerSecretRouter(svc: IContainerSecretService): OpenAPIHono {
  ...
  app.openapi(createRoute({ method: 'get', path: '/', tags: ['container-secrets'], summary: '列出 ContainerSecret', responses: { 200: { description: '{ items: ContainerSecret[] }', content: { 'application/json': { schema: OkResponse(ContainerSecretListResponseSchema) } } } } }), async (c) => {
    const scopeId = c.req.query('scopeId');
    const items = await svc.list(scopeId);
    return c.json(ok({ items: items.map(redact), total: items.length }));
  });

  app.openapi(createRoute({ method: 'get', path: '/{id}', tags: ['container-secrets'], summary: '获取 ContainerSecret 详情', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'ContainerSecret', content: { 'application/json': { schema: OkResponse(ContainerSecretResponseSchema) } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'put', path: '/{id}', tags: ['container-secrets'], summary: '更新 ContainerSecret', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'ContainerSecret', content: { 'application/json': { schema: OkResponse(ContainerSecretResponseSchema) } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'delete', path: '/{id}', tags: ['container-secrets'], summary: '删除 ContainerSecret', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Deleted', content: { 'application/json': { schema: OkResponse(z.null()) } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/public-key/{userId}', tags: ['container-secrets'], summary: '获取用户的 SealedBox 公钥', request: { params: z.object({ userId: z.string() }) }, responses: { 200: { description: '{ userId, publicKey, keyType }', content: { 'application/json': { schema: OkResponse(PublicKeyResponseSchema) } } } } }), async (c) => {
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/scopes', tags: ['container-secrets'], summary: '获取 secret 的 selectedScopeIds', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'string[]', content: { 'application/json': { schema: OkResponse(z.array(z.string())) } } } } }), async (c) => {
    const secret = await svc.get(c.req.param('id'));
    if (!secret) throw new AppError(404, 'SECRET_NOT_FOUND', 'Container secret not found');
    return c.json(ok(secret.selectedScopeIds));
  });

  app.openapi(createRoute({ method: 'put', path: '/{id}/scopes', tags: ['container-secrets'], summary: '设置 secret 的 selectedScopeIds', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'ContainerSecret', content: { 'application/json': { schema: OkResponse(ContainerSecretResponseSchema) } } } } }), async (c) => {
    const body = await z.unknown().parse(c.req.json());
  ...
  app.openapi(createRoute({ method: 'get', path: '/{id}/check-access', tags: ['container-secrets'], summary: '检查 scopeId 是否有权访问此 secret', request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: '{ allowed: boolean }', content: { 'application/json': { schema: OkResponse(CheckAccessResponseSchema) } } } } }), async (c) => {
    const scopeId = c.req.query('scopeId');
