...
import { VolumeSchema, VolumeListResponseSchema } from './response-schema.ts';
...
export function createVolumeRouter(svc: IVolumeService): OpenAPIHono<{ Variables: AppContext }> {
  ...
  app.openapi(
    createRoute({
      ...
      responses: { 201: { description: 'Volume created', content: { 'application/json': { schema: OkResponse(VolumeSchema) } } } },
    }),
    async (c) => {
      isRoot(c);
      const body = await CreateVolumeSchema.parse(c.req.json());
      const volume = await svc.create(z.custom<CreateVolumeInput>().parse(body));
  ...
  app.openapi(
    createRoute({
      ...
      tags: ['volumes'],
      summary: '列出数据卷',
      responses: { 200: { description: 'Volume[]', content: { 'application/json': { schema: OkResponse(VolumeListResponseSchema) } } } },
    }),
    async (c) => {
  ...
  app.openapi(
    createRoute({
      ...
      responses: { 200: { description: 'Volume', content: { 'application/json': { schema: OkResponse(VolumeSchema) } } } },
    }),
    async (c) => {
  ...
  app.openapi(
    createRoute({
      ...
      responses: { 200: { description: 'Volume updated', content: { 'application/json': { schema: OkResponse(VolumeSchema) } } } },
    }),
    async (c) => {
      isRoot(c);
      const body = await UpdateVolumeSchema.parse(c.req.json());      return c.json(ok(await svc.update(c.req.param('id'), body)));
    },
  );
