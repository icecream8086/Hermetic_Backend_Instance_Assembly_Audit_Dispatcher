...
describe('PermissionChecker', () => {
  ...
  describe('check', () => {
    it('returns denied when user does not exist', async () => {
      const r = await checker.check({ userId: 'missing', action: 'read', resource: 'pod' }, emptyMacRules);
    ...
    it('returns denied when user exists but no policies or groups grant access', async () => {
      ...
      const r = await checker.check({ userId: 'u1', action: 'read', resource: 'pod' }, emptyMacRules);
      expect(r.allowed).toBe(false);
    });
    ...
    it('denies when MAC rule denies and global policy allows (deny-overrides)', async () => {
      ...
      const r = await checker.check({ userId: 'u1', action: 'read', resource: 'pod' }, macRules);
      expect(r.allowed).toBe(false);
    });
    ...
    it('grants access via user group rule', async () => {
      ...
      const pg = { id: 'pg1', name: 'Dev Access', userGroupIds: ['ug1'], rules: [{ effect: 'allow' as const, actions: ['read'], resource: 'pod' }], createdAt: Date.now(), updatedAt: Date.now() };
    ...
    it('denies when action does not match any rule', async () => {
      ...
      checker.invalidateCache();
      const r = await checker.check({ userId: 'u1', action: 'delete', resource: 'pod' }, emptyMacRules);
      expect(r.allowed).toBe(false);
    });

    it('handles $self resource expansion', async () => {
      await atomic.set('user:u1', { id: 'u1', name: 'Test' }, null);
      const policy = { id: 'pol_self', name: 'Own Resources', effect: 'allow' as const, actions: ['*'], resource: 'pod:$self', priority: 1000, enabled: true, createdAt: Date.now(), updatedAt: Date.now() };
      ...
      const r = await checker.check({
        userId: 'u1', action: 'read', resource: 'pod:u1', resourceOwnerId: 'u1',
      }, emptyMacRules);
      expect(r.allowed).toBe(true);
    });
  });
  ...
  describe('cache', () => {
    it('serves from cache within TTL (5s)', async () => {
      await atomic.set('user:u1', { id: 'u1', name: 'Test' }, null);
      const policy = { id: 'pol_cache', name: 'Allow All', effect: 'allow' as const, actions: ['read'], resource: 'pod:*', priority: 1000, enabled: true, createdAt: Date.now(), updatedAt: Date.now() };
      ...
      const r1 = await checker.check({ userId: 'u1', action: 'read', resource: 'pod:123' }, emptyMacRules);
      expect(r1.allowed).toBe(true);

      // Second check: still within TTL → uses cache, same result
      const r2 = await checker.check({ userId: 'u1', action: 'read', resource: 'pod:123' }, emptyMacRules);
      expect(r2.allowed).toBe(true);

      // Invalidate: next check reloads from store
      checker.invalidateCache();
      // Policy still exists in store → still allowed
      const r3 = await checker.check({ userId: 'u1', action: 'read', resource: 'pod:123' }, emptyMacRules);
    ...
    it('invalidateCache causes fresh reload from store', async () => {
      ...
      // No policies → deny
      const r1 = await checker.check({ userId: 'u1', action: 'read', resource: 'pod' }, emptyMacRules);
      ...
      await atomic.set('policy:pol_new', { id: 'pol_new', name: 'New', effect: 'allow', actions: ['read'], resource: 'pod', priority: 1000, enabled: true, createdAt: Date.now(), updatedAt: Date.now() }, null);
      checker2.invalidateCache(); // force checker2 to reload

      // checker2 sees the new policy
      const r2 = await checker2.check({ userId: 'u1', action: 'read', resource: 'pod' }, emptyMacRules);
      expect(r2.allowed).toBe(true);

      // checker1 still has stale cached empty list
      const r1b = await checker.check({ userId: 'u1', action: 'read', resource: 'pod' }, emptyMacRules);
      expect(r1b.allowed).toBe(false); // stale cache

      // After checker1 invalidates, it also sees the policy
      checker.invalidateCache();
      const r3 = await checker.check({ userId: 'u1', action: 'read', resource: 'pod' }, emptyMacRules);
      expect(r3.allowed).toBe(true);
    });
  });
});
