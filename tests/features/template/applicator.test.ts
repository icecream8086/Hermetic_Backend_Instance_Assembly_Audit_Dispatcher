...
describe('applyTemplate', () => {
  it('maps name and region', () => {
    const r = applyTemplate(minimalTpl(), 'my-sandbox', 'cn-hangzhou');
    expect(r.name).toBe('my-sandbox');
    expect(r.region).toBe('cn-hangzhou');
  });

  it('defaults name to template.name when not overridden', () => {
    const r = applyTemplate(minimalTpl());
    expect(r.name).toBe('test-template');
  });

  it('defaults region to container.region', () => {
    const r = applyTemplate(minimalTpl());
    expect(r.region).toBe('local');
  });

  it('defaults spotStrategy to None', () => {
    const r = applyTemplate(minimalTpl());
    expect(r.spotStrategy).toBe('None');
  });

  it('defaults restartPolicy to Always', () => {
    const r = applyTemplate(minimalTpl());
    expect(r.restartPolicy).toBe('Always');
  });

  it('computes cpu sum from container limits', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
        containers: [
  ...
  it('computes cpu as 1 per container when no resources set', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
        containers: [
  ...
  it('computes memory sum from container limits', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
        containers: [
  ...
  it('defaults memory to 512 per container', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
  ...
  it('maps container name and image', () => {
    const r = applyTemplate(minimalTpl());
  ...
  it('maps container command and args', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
  ...
  it('maps container env with value', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
  ...
  it('maps container env with valueFrom', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
  ...
  it('defaults port protocol when not set', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
  ...
  it('maps container resources requests and limits', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
        containers: [{
  ...
  it('maps instanceId from container spec', () => {
    const r = applyTemplate(minimalTpl({
  ...
  it('maps description', () => {
    const r = applyTemplate(minimalTpl({ description: 'My template' }));
...
describe('applyTemplate health checks', () => {
  it('maps livenessProbe to container', () => {
    const r = applyTemplate(minimalTpl({
  ...
  it('maps readinessProbe and startupProbe simultaneously', () => {
    const r = applyTemplate(minimalTpl({
      container: { region: 'local' as any, containers: [{ name: 'web', image: 'nginx' }] },
      healthChecks: [
  ...
  it('maps probes to initContainers', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
        containers: [{ name: 'app', image: 'nginx' }],
  ...
  it('handles successThreshold and failureThreshold', () => {
    const r = applyTemplate(minimalTpl({
      container: { region: 'local' as any, containers: [{ name: 'web', image: 'nginx' }] },
      healthChecks: [{
...
describe('applyTemplate network', () => {
  it('defaults allocatePublicIp to false when network undefined', () => {
    const r = applyTemplate(minimalTpl({ network: undefined }));
    expect((r.network as any).allocatePublicIp).toBe(false);
  });

  it('maps publicIp.allocate', () => {
    const r = applyTemplate(minimalTpl({ network: { publicIp: { allocate: true } } }));
    expect((r.network as any).allocatePublicIp).toBe(true);
  });

  it('maps publicIp.bandwidth', () => {
    const r = applyTemplate(minimalTpl({ network: { publicIp: { allocate: true, bandwidth: 100 } } }));
    expect((r.network as any).publicIpBandwidth).toBe(100);
  });

  it('maps vpc securityGroupId (system UID)', () => {
    const r = applyTemplate(minimalTpl({ network: { vpc: { securityGroupId: 'sg_xxx' } } }));
    expect((r.network as any).securityGroupId).toBe('sg_xxx');
  });

  it('maps vpc subnetIds', () => {
    const r = applyTemplate(minimalTpl({ network: { vpc: { subnetIds: ['vsw-a', 'vsw-b'] } } }));
    expect((r.network as any).subnetIds).toEqual(['vsw-a', 'vsw-b']);
  });

  it('maps vpc instanceId', () => {
    const r = applyTemplate(minimalTpl({ network: { vpc: { instanceId: 'inst_xxx' as any } } }));
    expect((r.network as any).instanceId).toBe('inst_xxx');
  });

  it('combines publicIp and vpc settings', () => {
    const r = applyTemplate(minimalTpl({
      network: {
...
describe('mapStorage', () => {
  it('returns empty arrays for undefined storage', () => {
    const r = mapStorage(undefined);
    expect(r.volumes).toEqual([]);
    expect(r.volumeMounts).toEqual([]);
  });

  it('returns empty arrays for empty storage', () => {
    const r = mapStorage([]);
    expect(r.volumes).toEqual([]);
    expect(r.volumeMounts).toEqual([]);
  });
  ...
  it('maps NFS with readOnly flag', () => {
    const r = mapStorage([{
  ...
  it('skips NFS when nfs config is missing', () => {
    const r = mapStorage([{ name: 'broken', type: 'nfs', mountPath: '/x' } as any]);
    expect(r.volumes).toHaveLength(0);
  });

  it('maps hostPath storage', () => {
    const r = mapStorage([{ name: 'local', type: 'hostPath', mountPath: '/cache' }]);
    expect(r.volumes).toHaveLength(1);
    expect(r.volumes[0]!.type).toBe('HostPathVolume');
    expect(r.volumeMounts[0]!.mountPath).toBe('/cache');
  });

  it('maps emptyDir storage', () => {
    const r = mapStorage([{ name: 'tmp', type: 'emptyDir', mountPath: '/tmp' }]);
    expect(r.volumes).toHaveLength(1);
  ...
  it('skips OSS when oss config is missing', () => {
    const r = mapStorage([{ name: 'bad-oss', type: 'oss', mountPath: '/x' } as any]);
    expect(r.volumes).toHaveLength(0);
  });

  it('maps multiple storage entries', () => {
    const r = mapStorage([
  ...
  it('assigns volumeMounts to first container only', () => {
    const r = applyTemplate(minimalTpl({
      container: {
        region: 'local' as any,
...
describe('applyTemplate extensions', () => {
  it('maps spotStrategy', () => {
    const r = applyTemplate(minimalTpl({ extensions: { spotStrategy: 'SpotAsPriceGo' } }));
    expect(r.spotStrategy).toBe('SpotAsPriceGo');
  });

  it('maps healthMaxRetries', () => {
    const r = applyTemplate(minimalTpl({ extensions: { healthMaxRetries: 5 } }));
    expect(r.healthMaxRetries).toBe(5);
  });

  it('maps providerOverrides', () => {
    const r = applyTemplate(minimalTpl({ extensions: { providerOverrides: { key: 'val' } } }));
...
describe('applyTemplate edge cases', () => {
  it('handles template with no container block', () => {
    const r = applyTemplate(minimalTpl({ container: undefined as any }));
    expect(r.name).toBe('test-template');
  ...
  it('handles template with empty containers array', () => {
    const r = applyTemplate(minimalTpl({
  ...
  it('copies arrays to prevent mutation', () => {
  ...
  it('uses container.account when set', () => {
    const r = applyTemplate(minimalTpl({
  ...
  it('handles health checks targeting non-existent container gracefully', () => {
    const r = applyTemplate(minimalTpl({
      container: { region: 'local' as any, containers: [{ name: 'web', image: 'nginx' }] },
      healthChecks: [{
